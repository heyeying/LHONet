...
def test(model, path, dataset):
    ...
    for i in range(num1):
        ...
        res2, res3 = model(image)
        # eval Dice
        res = F.interpolate(res2 + res3, size=gt.shape, mode='bilinear', align_corners=False)
...
def train(train_loader, model, optimizer, epoch, test_path):
        ...
        if i % 20 == 0 or i == total_step:
            print('{} Epoch[{:03d}/{:03d}], Step[{:04d}/{:04d}],'
                  ...
                  format(datetime.now(), epoch, opt.epoch, i, total_step,
                         loss_record.show(), loss_P2, optimizer.param_groups[0]['lr']))
    ...
    if (epoch + 1) % 1 == 0 :
        ...
        if meandice > best:
            print('#'*80)
            best = meandice
            torch.save(model.state_dict(), save_path + 'LHONet-best.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', type=str,
                        default='./datasets/polyp/TrainDataset/',   # Training set path
                        help='path to train dataset')

    parser.add_argument('--test_path', type=str,        # Test Set Path
    ...
    torch.cuda.set_device(3)  # set your gpu device
    model = LHONet().cuda()
